...
def style_max_cells(s: pd.Series, color="#2DD4BF", bold=True, border=True, tol=1e-9):
    """Max in column (ties ok), tolerant to float rounding; apply this LAST."""
    numeric = pd.to_numeric(s, errors="coerce")
    vmax = numeric.max()
    out = []
    for raw, val in zip(s, numeric):
...
def format_confidence_badge(sig: SignalOutput):
    if sig.side=="CALL": color="#9ACD32"; emoji="🟢"
    elif sig.side=="PUT": color="#9ACD32"; emoji="🔴"
    ...
    try:
        ...
        if df_ce.empty or df_pe.empty:
            ...
            styler = (tbl[show_cols].style
                      ...
                      .apply(highlight_atm_row, axis=1)                # row background
                      .apply(highlight_strike_col, subset=["Strike"])  # strike column tint
                     )

            # recommended strike: border only → won't override max cell bg
            ...
            # 🔚 Apply max-highlighting LAST so it wins
            ...
            try:
                st.session_state.snapshots.append({
                    "ts": meta, "symbol": symbol, "spot": spot_price, "mp": mp, "df": df_oi.copy(),
                })
